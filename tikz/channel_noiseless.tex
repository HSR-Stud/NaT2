\begin{tikzpicture}
	...
	\node (x3) at(0,-3) [dot] {}  (x3) node[anchor=east] {$x_m$};

	\node (y1) at(5,0) [dot] {}  (y1) node[anchor=west] {$y_1$};
	\node (y2) at(5,-1)[dot] {} (y2) node[anchor=west] {$y_2$};
	\node (yy) at(5,-2)[] {} (yy) node[anchor=west] {\vdots};
	\node (y3) at(5,-3)[dot] {} (y3) node[anchor=west] {$y_m$};
